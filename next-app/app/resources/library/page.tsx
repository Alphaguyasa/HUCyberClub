...
export default function ResourceLibraryPage() {
  ...
  return (
    <div className={`relative ${spectral.className}`} style={{ backgroundColor: '#FFFEF0' }}>

      {/* Header */}
      <header className="w-full fixed top-0 z-50 transition-all duration-300"
        style={{ backgroundColor: scrolled ? '#FEFB8F' : '#FFFEF0', boxShadow: scrolled ? '0 1px 8px rgba(0,0,0,0.08)' : 'none' }}>
        <div className="max-w-full px-8 py-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-base" style={{ color: '#111111' }}>
          ...
          <nav className="hidden lg:flex gap-6">
            {["Home", "About", "Resources", "Events", "Contact"].map((item) => (
              <Link key={item}
                ...
                style={{ color: '#111111', fontSize: '15px', borderBottom: item === "Resources" ? '2px solid #111111' : 'none', fontWeight: item === "Resources" ? 600 : 400 }}
              >{item}</Link>
            ))}
          </nav>
      ...
      <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="font-light mb-4" style={{ fontSize: '42px', color: '#1D2023' }}>
            Resource Library
          </h1>
          <p className="font-light" style={{ fontSize: '18px', color: '#C9C73C' }}>
            Browse our full collection of cybersecurity resources, tools, and projects.
          </p>
        </div>
